...
test("wildcard worker devuelve una plantilla factible y serializable", async () => {
  ...
  for (const [pos, count] of [["GK", 4], ["DEF", 10], ["MID", 10], ["FWD", 8]]) {
    for (let index = 0; index < count; index += 1) players.push({ id: players.length + 1, pos, club: `${pos}${index % 5}`, price: pos === "FWD" ? 7 : 5, confidence: 90 });
  }
  const scores = players.map((player) => ({ id: player.id, value: player.pos === "FWD" ? 8 : 5 }));
  context.self.onmessage({ data: { type: "optimize", players, scores, budget: 100, quotas: { GK: 2, DEF: 5, MID: 5, FWD: 3 }, formations: [[3, 5, 2], [4, 4, 2], [3, 4, 3]] } });
